...
def sample(env,
           ...
           verbose=False):
    """
        Write a sampler function which takes in an environment,
        a controller (either random or the MPC controller),
        and returns rollouts by running on the env.
        Each path can have elements for observations, next_observations,
...
def path_cost(cost_fn, path):
    ...
    return trajectory_cost_fn(
        cost_fn, path['observations'], path['actions'],
...
def compute_normalization(data):
    ...
    """
    observations = np.concatenate([d['observations'] for d in data])
    next_observations = np.concatenate([d['next_observations'] for d in data])
...
def plot_comparison(env, dyn_model):
    """
    Write a function to generate plots comparing the behavior of the
    model predictions for each element of the state to the actual ground truth,
    using randomly sampled actions.
    """
    # paths = sample(env, dyn_model)
    pass
...
def train(env,
    ...
    data = sample(env, random_controller,
                  num_paths_random, env_horizon)
    ...
    for itr in range(onpol_iters):
        ...
        costs = [path_cost(cost_fn, ds) for ds in data_sample]
        logz.log_tabular('AverageCost', np.mean(costs))
        logz.log_tabular('StdCost', np.std(costs))
        logz.log_tabular('MinimumCost', np.min(costs))
        logz.log_tabular('MaximumCost', np.max(costs))
        # In terms of true environment reward of your rolled out
        # trajectory using the MPC controller
        returns = list([ds['rewards'] for ds in data_sample])
